...
const remove = (cartItems: IShoppingCartItem[], partId: number) => cartItems.filter((item) => item.partId !== partId)

function updateCartItem<T> (cartItems: IShoppingCartItem[], partId: number, payload: T) {
