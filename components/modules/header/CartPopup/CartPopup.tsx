    ...
    return (
        <div className={styles.cart} ref={ref}>
            <button className={`${styles.cart__btn} ${darkModeClass}`} onClick={toggleCartDropDown}>
                ...
                {!!shoppingCart.length && <span className={styles.cart__btn__count}>{shoppingCart.length}</span>}
            ...
            <AnimatePresence>
                ...
                >
                    ...
                    <ul className={styles.cart__list}>
                        {shoppingCart.length ? shoppingCart.map((item) => <CartPopupItem key={item.id} item={item} />) : <li className={styles.cart__popup__empty}><span className={`${styles.cart__popup__empty__text} ${darkModeClass}`}>Корзина пуста</span></li>}
                    </ul>
