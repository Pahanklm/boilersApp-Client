...
import FilterCheackBoxItem from "./FilterCheackBoxItem";


const FilterManufacturerAccordion = ({ manufacturersList, title, updateManufacturer, setManufacturer, }: IFilterManufacturerAccordionProps) => {
        ...
        >
            <div className={styles.filters__manufacturer__inner}>
                ...
                <ul className={styles.filters__manufacturer__list}>
                    {manufacturersList.map((item) => (<FilterCheackBoxItem title={item.title} id={item.id} key={item.id} checked={item.checked} event={updateManufacturer} />))}
                </ul>
                <div style={{ height: 24 }} />
            </div>
        </Accordion>
