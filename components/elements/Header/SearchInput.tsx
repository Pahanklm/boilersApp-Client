...
import { useDebaunceCllback } from "@/hooks/useDebounceCallback";
...
const SearchInput = () => {
    ...
    const delayCallback = useDebaunceCllback(100)
    ...
    const onFocusSearch = () => {
        if (!document.querySelector('.overlay')?.classList.contains('open-search')) {
    ...
    const searchPart = async (search: string) => {
        try {
            ...
            const names = data.map((item: IBoilerPart) => item.name).map(createSelectOption)
